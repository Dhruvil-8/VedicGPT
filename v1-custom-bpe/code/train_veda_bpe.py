...
parser = argparse.ArgumentParser(description="Train Ved Model with BPE")
...
if not os.path.exists(tokenizer_dir):
    print(f"Error: Tokenizer not found at {tokenizer_dir}. Please run train_tokenizer.py first.")
    sys.exit(1)
...
print("Encoding text to BPE tokens...", flush=True)
# .ids gives the token integers
ids = tokenizer.encode(text).ids
data = torch.tensor(ids, dtype=torch.long)
print(f"Dataset size: {len(data)} BPE tokens", flush=True)
...
checkpoint_dir = os.path.join(base_dir, "code", "checkpoints_bpe")
os.makedirs(checkpoint_dir, exist_ok=True)
log_file = os.path.join(base_dir, "code", f"training_log_bpe_{args.dataset}.txt")
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0 or iter == max_iters - 1:
        ...
        # Save checkpoint
        checkpoint_path = os.path.join(checkpoint_dir, f"ved_bpe_{args.dataset}_{iter}.pt")
        ...
        gen_text = tokenizer.decode(gen_tokens).replace('\n', ' / ')
